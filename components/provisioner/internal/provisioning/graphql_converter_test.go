		...
		expectedRuntimeStatus := &gqlschema.RuntimeStatus{
			...
			RuntimeConfiguration: &gqlschema.RuntimeConfig{
				ClusterConfig: &gqlschema.GardenerConfig{
					...
					TargetSecret:                        &secret,
					WorkerCidr:                          &workerCidr,
					PodsCidr:                            util.PtrTo(podsCIDR),
					ServicesCidr:                        util.PtrTo(servicesCIDR),
					AutoScalerMax:                       &autoScMax,
					AutoScalerMin:                       &autoScMin,
					MaxSurge:                            &surge,
					MaxUnavailable:                      &unavailable,
		...
		Components: []*gqlschema.ComponentConfiguration{
			...
			},
			{
				...
				Configuration: []*gqlschema.ConfigEntry{
					fixGQLConfigEntry("test.config.key", "value", util.PtrTo(false)),
					fixGQLConfigEntry("test.config.key2", "value2", util.PtrTo(false)),
				},
			},
			{
			...
			},
			{
				...
				Configuration: []*gqlschema.ConfigEntry{
					fixGQLConfigEntry("test.config.key", "value", util.PtrTo(false)),
					fixGQLConfigEntry("test.secret.key", "secretValue", util.PtrTo(true)),
				},
			},
		},
		Configuration: []*gqlschema.ConfigEntry{
			fixGQLConfigEntry("global.config.key", "globalValue", util.PtrTo(false)),
			fixGQLConfigEntry("global.config.key2", "globalValue2", util.PtrTo(false)),
			fixGQLConfigEntry("global.secret.key", "globalSecretValue", util.PtrTo(true)),
		},
	}
}
