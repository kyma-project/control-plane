...
func TestGet(t *testing.T) {
	...
	callsSuccess, err := commons.GardenerCalls.GetMetricWithLabelValues("success", existingShoot, "success_getting_secret")
	g.Expect(err).Should(gomega.BeNil())
	...
	callsFailure, err := commons.GardenerCalls.GetMetricWithLabelValues("failure", nonexistentShoot, "failed_getting_secret")
	g.Expect(err).Should(gomega.BeNil())
