...
func (c Client) Get(ctx context.Context, shootName string) (*corev1.Secret, error) {
	...
	if err != nil {
		gardenercommons.TotalCalls.WithLabelValues("failure", shootName, "failed_getting_secret").Inc()
		return nil, err
	}
	gardenercommons.TotalCalls.WithLabelValues("success", shootName, "success_getting_secret").Inc()
