...
func (c Client) Get(ctx context.Context, shootName string) (*gardenerv1beta1.Shoot, error) {
	...
	if err != nil {
		gardenercommons.GardenerCalls.WithLabelValues("failure", shootName, "failed_getting_shoot").Inc()
		return nil, err
	}
	gardenercommons.GardenerCalls.WithLabelValues("success", shootName, "success_getting_shoot").Inc()
