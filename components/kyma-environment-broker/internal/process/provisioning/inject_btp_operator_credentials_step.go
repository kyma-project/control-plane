...
func (s *InjectBTPOperatorCredentialsStep) Run(operation internal.Operation, log logrus.FieldLogger) (internal.Operation, time.Duration, error) {
	...
	secret := s.handler.PrepareSecret(operation.ProvisioningParameters.ErsContext.SMOperatorCredentials, clusterID)
	if err := s.handler.CreateOrUpdateSecret(operation.K8sClient, secret, log); err != nil {
