...
func (s *InjectBTPOperatorCredentialsStep) createOrUpdateSecret(k8sClient client.Client, parametersBasedSecret *apicorev1.Secret, log logrus.FieldLogger) error {
	...
	if err != nil {
		log.Errorf("failed to update the secret for BTP Manager: %w", err)
		return err
	}
	...
	if apierrors.IsNotFound(err) {
		...
		if err != nil {
			if apierrors.IsAlreadyExists(err) {
				log.Warnf("could not create %s namespace: %w - the namespace already exists", secretNamespace)
			} else {
				log.Warnf("could not create %s namespace: %w - ignoring", secretNamespace, err)
			}
		}
