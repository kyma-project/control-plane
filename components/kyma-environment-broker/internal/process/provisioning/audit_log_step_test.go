...
func TestAuditLog_ScriptFileDoesNotExist(t *testing.T) {
	...
	kymaVersion := "1.18.1"
	svc := NewAuditLogOverridesStep(repo, cfg, kymaVersion)
	svc.fs = mm
...
func TestAuditLog_HappyPath_HTTP_Plugin(t *testing.T) {
	// given
	mm := afero.NewMemMapFs()
...
func TestAuditLog_HappyPath_Seq_HTTP_Plugin(t *testing.T) {
	// given
	mm := afero.NewMemMapFs()
