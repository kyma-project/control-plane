...
func (s *ConnUnbindStep) Run(operation internal.DeprovisioningOperation, log logrus.FieldLogger) (internal.DeprovisioningOperation, time.Duration, error) {
	...
	// Unbind
	log.Infof("unbinding for Connectivity instance: %s started; binding: %s", operation.Conn.Instance.InstanceID, operation.Conn.Binding.BindingID)
	_, err = smCli.Unbind(operation.Conn.Instance.InstanceKey(), operation.Conn.Binding.BindingID, true)
	if err != nil {
		return s.handleError(operation, err, log, fmt.Sprintf("unable to unbind, bindingId=%s", operation.Conn.Binding.BindingID))
	}
	...
	updatedOperation, retry := s.operationManager.UpdateOperation(operation, func(operation *internal.DeprovisioningOperation) {
		operation.Conn.Binding.BindingID = ""
