...
func TestLmsStepsHappyPath(t *testing.T) {
	...
	createTenanat := lms.CreateTenantInput{
		Name:            "foo-bar",
		Region:          "eu",
		GlobalAccountID: "foo-global-id",
	}

	lmsTen, err := lmsClient.CreateTenant(createTenanat)
	require.NoError(t, err)
	...
	opRepo.InsertUpgradeKymaOperation(operation)
	...
	// when
	op, when, err := certStep.Run(operation, fixLogger())
