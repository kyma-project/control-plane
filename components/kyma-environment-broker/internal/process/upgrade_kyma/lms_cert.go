...
const (
	...
	upgradekibanaURLLabelKey = "operator_lmsUrl"
)
...
func (s *lmsCertStep) Name() string {
	return "Upgrade_Request_LMS_Certificates"
}
...
func (s *LMSUpgradeStep) handleError(operation internal.UpgradeKymaOperation, log logrus.FieldLogger, since time.Duration, msg string, err error) (internal.UpgradeKymaOperation, time.Duration, error) {
...
func (s *LMSUpgradeStep) failLmsAndUpdate(operation internal.UpgradeKymaOperation, msg string, log logrus.FieldLogger) (internal.UpgradeKymaOperation, time.Duration, error) {
	operation.Lms.Failed = true
	if s.isMandatory {
		return s.operationManager.OperationFailed(operation, msg, log)
	}
	modifiedOp, retry := s.operationManager.UpdateOperation(operation, func(operation *internal.UpgradeKymaOperation) {
		operation.Lms.Failed = true
	}, log)
