...
func TestConnectivityEncryptDecrypt(t *testing.T) {
	...
	givenOverrides := provisioning.ConnectivityConfig{
		ClientId:     "clientid",
	...
	// when
	encryptedOverrides, err := provisioning.EncryptConnectivityConfig(secretKey, &givenOverrides)
	assert.NoError(t, err)
	decryptedOverrides, err := provisioning.DecryptConnectivityConfig(secretKey, encryptedOverrides)
	assert.NoError(t, err)
