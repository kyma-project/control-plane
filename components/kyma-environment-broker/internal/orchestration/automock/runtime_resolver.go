...
func (_m *RuntimeResolver) Resolve(targets internal.TargetSpec) ([]internal.Runtime, error) {
	ret := _m.Called(targets)

	var r0 []internal.Runtime
	if rf, ok := ret.Get(0).(func(internal.TargetSpec) []internal.Runtime); ok {
		r0 = rf(targets)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(internal.TargetSpec) error); ok {
		r1 = rf(targets)
	} else {
		r1 = ret.Error(1)
	}
