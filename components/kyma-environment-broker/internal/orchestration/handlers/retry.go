...
func (r *Retryer) kymaUpgradeRetry(o *internal.Orchestration, opsByOrch []internal.UpgradeKymaOperation, operationIDs []string) error {
	...
	ops, invalidIDs, err = r.kymaOperationsDeprovisionedFilter(ops)
	if err != nil {
		return err
	}
	r.resp.InvalidOperations = append(r.resp.InvalidOperations, invalidIDs...)
	if len(ops) == 0 {
		r.zeroValidOperationInfo(o.OrchestrationID)
		return nil
	}
...
// filter out the deprovision (no matter in progrss/failed/succeeded) or suspension operations
func (r *Retryer) kymaOperationsDeprovisionedFilter(ops []internal.UpgradeKymaOperation) ([]internal.UpgradeKymaOperation, []string, error) {
	...
	for _, op := range ops {
		...
		kymaOps, err := r.operations.GetDeprovisioningOperationByInstanceID(instanceID)
		if err != nil && !dberr.IsNotFound(err) {
			r.log.Errorf("while getting deprovisioning for instance with ID: %s: %v", instanceID, err)
			err = errors.Wrapf(err, "while getting deprovisioning for instance with ID %s", instanceID)
			return nil, nil, err
		}
		if kymaOps != nil {
			// deprovision or suspension
			invalidIDs = append(invalidIDs, op.Operation.ID)
			continue
		}
	...
	return retryOps, invalidIDs, nil
}
...
func (r *Retryer) clusterUpgradeRetry(o *internal.Orchestration, opsByOrch []internal.UpgradeClusterOperation, operationIDs []string) error {
	...
	ops, invalidIDs, err = r.clusterOperationsDeprovisionedFilter(ops)
	if err != nil {
		return err
	}
	r.resp.InvalidOperations = append(r.resp.InvalidOperations, invalidIDs...)
...
// filter out the deprovision (no matter in progrss/failed/succeeded) or suspension operations
func (r *Retryer) clusterOperationsDeprovisionedFilter(ops []internal.UpgradeClusterOperation) ([]internal.UpgradeClusterOperation, []string, error) {
