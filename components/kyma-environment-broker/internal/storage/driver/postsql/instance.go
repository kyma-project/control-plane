...
func (s *Instance) toInstance(dto dbmodel.InstanceDTO) (internal.Instance, error) {
	...
	if strings.Index(params.Parameters.Kubeconfig, "kind:") == -1 {
		err = s.cipher.DecryptKubeconfig(&params)
		if err != nil {
			return internal.Instance{}, errors.Wrapf(err, "while decrypting kubeconfig starting with %s", params.Parameters.Kubeconfig[0:9])
		}
	} else {
