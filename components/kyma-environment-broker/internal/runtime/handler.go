...
func (h *Handler) determineStatusModifiedAt(dto *pkg.RuntimeDTO) error {
	...
	if err != nil && !dberr.IsNotFound(err) {
		return fmt.Errorf("while fetching last operation for instance %s", dto.InstanceID)
	}
	if last != nil {
...
func (h *Handler) setRuntimeAllOperations(instance internal.Instance, dto *pkg.RuntimeDTO) error {
	...
	if err != nil && !dberr.IsNotFound(err) {
		return fmt.Errorf("while fetching provisioning operations list for instance %w", err)
	}
	if len(provOprs) != 0 {
	...
	if err != nil && !dberr.IsNotFound(err) {
		return fmt.Errorf("while fetching deprovisioning operations list for instance %w", err)
	}
	...
	if err != nil && !dberr.IsNotFound(err) {
		return fmt.Errorf("while fetching upgrade kyma operation for instance %w", err)
	}
	...
	if err != nil && !dberr.IsNotFound(err) {
		return fmt.Errorf("while fetching upgrade cluster operation for instance %w", err)
	}
	...
	if err != nil && !dberr.IsNotFound(err) {
		return fmt.Errorf("while fetching update operation for instance %w", err)
	}
...
func (h *Handler) setRuntimeLastOperation(instance internal.Instance, dto *pkg.RuntimeDTO) error {
	...
	if err != nil {
		return fmt.Errorf("while fetching last operation instance %w", err)
	}
	...
	case internal.OperationTypeProvision:
		...
		if err != nil {
			return fmt.Errorf("while fetching provisioning operations for instance %w", err)
		}
	...
	case internal.OperationTypeDeprovision:
		...
		if err != nil {
			return fmt.Errorf("while fetching deprovisioning operation for instance %w", err)
		}
...
func (h *Handler) setRuntimeOptionalAttributes(instance internal.Instance, dto *pkg.RuntimeDTO, kymaConfig, clusterConfig bool) error {
	if kymaConfig || clusterConfig {
		...
		if err != nil && !dberr.IsNotFound(err) {
			return fmt.Errorf("while fetching runtime states for instance %w", err)
		}
